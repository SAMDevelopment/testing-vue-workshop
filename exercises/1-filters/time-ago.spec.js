...
describe('timeAgo', () => {
  ...
  test('returns plural minutes', () => {
    expect(timeAgo(unixTime - minutes(2)))
      .toBe('2 minutes ago')
  })

  test('returns singular hour', () => {
    expect(timeAgo(unixTime - hours(1)))
      .toBe('1 hour ago')
  })

  test('returns plural hours', () => {
    expect(timeAgo(unixTime - hours(2)))
      .toBe('2 hours ago')
  })

  test('returns singular day', () => {
    expect(timeAgo(unixTime - days(1)))
      .toBe('1 day ago')
  })

  test('returns plural days', () => {
    expect(timeAgo(unixTime - days(2)))
      .toBe('2 days ago')
  })

  test('returns day rounded to nearest value', () => {
    expect(timeAgo(unixTime - days(1.2)))
      .toBe('1 day ago')

    expect(timeAgo(unixTime - days(1.8)))
      .toBe('2 days ago')
  })
})
